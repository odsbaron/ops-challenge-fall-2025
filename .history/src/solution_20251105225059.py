...
class ops:
    ...
    def rolling_regbeta(col_x_or_expr, col_y_or_expr, window: int) -> pl.Expr:
        ...
        return pl.when(var_x < 1*1e-9).then(0.0).otherwise(cov_xy / var_x).alias("rolling_regbeta")
...
def ops_rolling_regbeta(input_path: str, window: int = 20) -> np.ndarray:
    ...
    # 🌟 **修改在这里！** 🌟
    # 在转换为 NumPy 数组之前，我们先把 "rolling_regbeta" 这一列转换为 Float64
    res_float64 = res.with_columns(
        pl.col("rolling_regbeta").cast(pl.Float64)
    )
    
    # 确保返回的是 np.float64 类型的数组
    return res_float64.to_numpy()